...
export function Sokoban({ onClose }: SokobanProps = {}) {
  ...
  const bundleRef = useRef<Nullable<LoadedLevels>>(null)
  ...
  const [state, setState] = useState<Nullable<SokobanState>>(null)
  const [loadError, setLoadError] = useState<Nullable<string>>(null)
  const [loading, setLoading] = useState(true)
  const [cellPx, setCellPx] = useState(CELL_MIN)
  const [heldDir, setHeldDir] = useState<Nullable<Direction>>(null)
