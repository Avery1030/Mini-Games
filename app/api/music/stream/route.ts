...
import { AUDIUS_TRACK_ID_RE, fetchAudiusAudio, fetchAudiusTrackStreamInfo } from '@/lib/music'
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    if (!upstream) {
      return NextResponse.json({ error: '音源不可用（节点拒绝或版权限制），请换一首或添加本地文件' }, { status: 403 })
    }
